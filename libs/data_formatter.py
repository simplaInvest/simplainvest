...
def format_central_captura(df_central_captura):
    ...
    df_central_captura['CAP DATA_CAPTURA'] = pd.to_datetime(df_central_captura['CAP DATA_CAPTURA']).dt.date
...
def format_grupos_wpp(df_grupos_wpp):
    ...
    if 'Data' in df_grupos_wpp.columns:
        if df_grupos_wpp['Data'].dtype == 'object':  # Apenas se ainda for texto
            ...
            df_grupos_wpp['Data'] = pd.to_datetime(df_grupos_wpp['Data'], format='%d/%m/%Y %H:%M:%S', errors='coerce')
